...
void mmid_sub_task(const double dt, const double endT, const long n) {
...
#ifdef PLOTS
    plots = createPlots("plot", "t u v", 0, n);
#endif
#ifdef ENERGY_CHECK
    ...
    for (double t = 0; t < endT; t += dt, step = (step + 1) % 2) {
        /*double result[n], ones[n];
        for (int l = 0; l < n; l++)
            ones[l] = 1;
        cblas_dgemv(CblasRowMajor, CblasNoTrans, (const int) n, (const int) n, 1, &k[0][0],
                    (const int) n,
                    u[step], 1, 0,
                    result, 1);
        double sumE_ = 0.5 * cblas_ddot((const int) n, result, 0, u[step], 0);
        cblas_dgemv(CblasRowMajor, CblasNoTrans, (const int) n, (const int) n, 1, &k[0][0],
                    (const int) n,
                    u[step], 1, 0,
                    result, 1);
        double sumU_ = cblas_ddot((const int) n, result, 0, ones, 0);
        cblas_dgemv(CblasRowMajor, CblasNoTrans, (const int) n, (const int) n, 1, &k[0][0],
                    (const int) n,
                    u[step], 1, 0,
                    result, 1);
        double sumV = 0.5 * cblas_ddot((const int) n, result, 0, u[step], 0);*/
        for (long i = 0; i < n; i++) {
...
#ifdef PLOTS
            fprintf(plots[i], "%+24.10lf %+24.10lf %+24.10lf\n", t, u[step][i], v[step][i]);
#endif
        }
#ifdef ENERGY_CHECK
